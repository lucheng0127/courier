...
定义基于角色的访问控制（RBAC）系统，区分管理员和普通用户的权限。
...
#### Scenario: 更新/删除 Provider

- **GIVEN** 用户已登录
- **AND** 用户角色为 `user`
- **WHEN** 发送 PUT/DELETE 请求到 `/api/v1/providers/:name`
- **THEN** 返回 403 状态码
- **AND** 响应体包含权限错误信息

#### Scenario: Provider 运维操作

- **GIVEN** 用户已登录
- **AND** 用户角色为 `user`
- **WHEN** 发送 POST 请求到 `/api/v1/admin/providers/reload` 或其他运维接口
- **THEN** 返回 403 状态码
...
User 模型 SHALL 包含角色字段。

#### Scenario: 用户模型结构

- **WHEN** 查看用户模型定义
- **THEN** User 结构体包含 `Role` 字段
- **AND** 字段类型为 `string`
- **AND** JSON 标签为 `role`
- **AND** 数据库标签为 `role`

### Requirement: Admin API Key 废弃

原有的 Admin API Key 鉴权方式 SHALL 被废弃，由 JWT + 角色系统替代。

#### Scenario: 过渡期兼容

- **GIVEN** 系统配置启用了过渡期兼容模式
- **WHEN** 使用旧的 Admin API Key 访问接口
- **THEN** 仍然可以访问（用于平滑迁移）

#### Scenario: 完全迁移后

- **GIVEN** 系统已完成迁移
- **WHEN** 使用 Admin API Key 访问接口
- **THEN** 返回 401 状态码
- **AND** 提示使用新的登录方式获取 JWT Token
